...
export function HeroSection({
  ...
  state: SurfaceState
  renderIn: 'page' | 'canvas'
  setRenderIn: React.Dispatch<React.SetStateAction<'page' | 'canvas'>>
  holderRef: HolderRef
  landRef: LandRef
  supported: boolean
  reduced: boolean
          ...
          <div ref={holderRef} className="home-hero-holder" style={{ transform: `scale(${scale})` }} hidden={preview}>
            {supported ? (
              <Surface
                ...
                renderIn={renderIn}
                size={[HERO_W, HERO_H]}
                timing={{ settleMs: reduced ? 0 : 120, durationMs: reduced ? 0 : 260 }}
                source={content}
              >
                <Surface.DOM />
              </Surface>
        ...
        <div className="home-hero-row">
          {supported ? (
            <button
              ...
              data-relief="raised"
              onClick={() => {
                if (!lifted) setRenderIn('canvas')
                else if (reduced) setRenderIn('page')
          ...
          <span className="home-lamp" data-gl={state.presented === 'canvas'} aria-live="polite">
            {preview ? 'Recorded in Chrome' : state.presented === 'canvas' ? 'In the scene' : 'On the page'}
          </span>
        </div>
...
// World px map 1:1 to CSS px at the page plane (PixelPerfect), with the
// origin at the viewport's centre and y up; the shader wants y down.
function publishFlyer(group: THREE.Group, size: { width: number; height: number }, lift: number) {
  group.updateMatrixWorld(true)
  FLYER_LOCAL.forEach(([x, y], index) => {
    flyerCorner.set(x, y, 0)
    group.localToWorld(flyerCorner)
    flyerCorners[index * 3] = flyerCorner.x + size.width / 2
    flyerCorners[index * 3 + 1] = size.height / 2 - flyerCorner.y
...
export function HeroMesh({
  ...
  surface: SurfaceHandle
  holderRef: HolderRef
  /** True once the mesh actually has the pixels — the launch waits for it. */
  presented: boolean
  requested: 'page' | 'canvas'
  reduced: boolean
  landRef: LandRef
  onLanded: () => void
}) {
  ...
  useLayoutEffect(() => {
    ...
    st.t = 0
    st.rx = st.ry = st.rz = 0
    st.landed = requested === 'page'
    landRef.current = false
  }, [requested, reduced, landRef])
  ...
  useFrame((_, delta) => {
    ...
    // The slot, in canvas coords (canvas IS the window), re-read per
    // frame so scrolling carries the whole flight.
    const r = holder.getBoundingClientRect()
    const sx = r.left + r.width / 2 - size.width / 2
    const sy = size.height / 2 - (r.top + r.height / 2)
    ...
    if (reduced) {
      ...
      if (requested === 'canvas' && !st.landed) publishFlyer(group, size, 0)
      return
    }
    st.clock += delta
    ...
    advancePhase(st, landRef, presented && requested === 'canvas', onLanded, sx, sy)

    const travel = Math.max(0, Math.min(32, r.left - 12, size.width - r.right - 12))
    ...
    if (requested === 'canvas' && !st.landed) publishFlyer(group, size, liftProgress(st))
  })
