...
export const lampObserver={name:'lamp-observer',enforce:'pre',transform(code,id){
  if(id.endsWith('/homeLightBulb.ts')){
    ...
    code=code.replace(marker,'  window.__lamp={group,body,cord,uniforms};\n'+marker)
    code=code.replace('      group.visible=true;','      if(window.__freezeLamp)return;\n      group.visible=true;')
  }
  if(id.endsWith('/HomeMasthead.tsx')){
    ...
    code=code.replace(marker,'window.__lampGpuStart?.();\n          '+marker+';\n          window.__lampGpuEnd?.();\n          window.__lampRendered?.(state.bulb.renderer.domElement)')
    code=code.replace('    pass.bulb = bulb','    window.__lampRenderer=renderer;\n    window.__lampRedraw=()=>state.draw();\n    pass.bulb = bulb')
  }
  if(id.endsWith('/homeLampBackdrop.ts'))code=code.replace('  return {\n    uniforms,','  window.__lampPaintCount=()=>source?.paintCount()??0;\n  return {\n    uniforms,')
  return code
}}
