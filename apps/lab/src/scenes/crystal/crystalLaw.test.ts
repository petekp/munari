...
describe('the solid', () => {
  ...
  it('is a solid all the way through, not a bump on a plane', () => {
    ...
    const mid: Vec3 = [40, 90, (TOP + KEEL) / 2]
    expect(sdInner2(mid[0], mid[1], tune)).toBeLessThan(-tune.girdlePx)
    expect(sdCrystal(mid, tune)).toBeLessThan(0)
    expect(sdCrystal([mid[0], mid[1], TOP + 1], tune)).toBeGreaterThan(0)
    expect(sdCrystal([mid[0], mid[1], KEEL - 1], tune)).toBeGreaterThan(0)
  })
  ...
  it('hangs its underside BELOW the hotspot, which is what a pavilion is', () => {
    ...
    expect(bottomAt(sdInner2(40, 130, tune), tune)).toBeLessThan(-6)
  ...
  it('bends every ray on the way OUT, which a flat bottom could not', () => {
    ...
    expect(bottomAt(sdInner2(40, 130, flat), flat)).toBeCloseTo(0, 6)
  ...
  it('is a lens over its whole face, which a flat top could not be', () => {
    ...
    // = 41.6px in from the silhouette, so 88% of this outline is within that
    ...
    expect(facet / (facet + table)).toBeCloseTo(0.88, 2)
  ...
  it('keeps the two angles inside the budget that lets the page through', () => {
    ...
    //
    // Measured 2026-08-25 at the hotspot: 30 + 15 gives 27.0 degrees and
    // every ray goes through; 30 + 20 gives 38.6 and the correction dies at
    // the right of the screen; 50 + 24 traps 51% of the light and reports no
    // answer at three of four screen positions.
    const critical = (Math.asin(1 / tune.ior) * 180) / Math.PI
    expect(critical).toBeCloseTo(38.1, 1)
    const rad = (tune.crownDeg * Math.PI) / 180
    const deflect = tune.crownDeg - (Math.asin(Math.sin(rad) / tune.ior) * 180) / Math.PI
    expect(deflect).toBeCloseTo(12.0, 1)
  ...
  it('points its normals OUT of the stone, which is what makes a bounce a bounce', () => {
    ...
    //
    // (51,129) is the deepest point of the arrow's outline, 67.5px inside
    // it — the only kind of place wide enough to still be under the table,
    // since the crown facet eats 41.6px of width before the table starts.
    const deep = sdInner2(51, 129, tune)
    expect(deep).toBeCloseTo(-67.5, 1)
    const nTable = normalAt([51, 129, topAt(deep, tune) - 0.2], tune)
    expect(nTable[2]).toBeGreaterThan(0.99)
    const nUnder = normalAt([51, 129, bottomAt(deep, tune) + 0.2], tune)
    expect(nUnder[2]).toBeLessThan(-0.9)

    // On a crown facet it leans out by exactly the crown angle. The pavilion
    // is steeper per px and never runs out of width, so the arrow has no
    // flat culet at all — it closes to a keel, which is why the underside
    // above reads as a facet and not as a floor.
    const nFacet = normalAt([10, 30, topAt(sdInner2(10, 30, tune), tune) - 0.2], tune)
    expect((Math.acos(nFacet[2]) * 180) / Math.PI).toBeCloseTo(tune.crownDeg, 1)
    expect(tune.pavilionPx / Math.tan((tune.pavilionDeg * Math.PI) / 180)).toBeGreaterThan(111)
  })
...
describe('the hotspot', () => {
  it('sits under the ground face and not under a vertex', () => {
    ...
    // `girdlePx` is where the silhouette sits, so the tip is 8.5px inside
    // the girdle — on a crown facet, well clear of the girdle band.
    expect(sdInner2(0, 0, tune)).toBeCloseTo(tune.chamferPx - tune.roundPx, 9)
    expect(tune.girdlePx - (tune.chamferPx - tune.roundPx)).toBe(8.5)
  ...
  it('is ON the glass, which a pavilion does not give for free', () => {
    ...
    expect(hotspotDrop(tune)).toBeCloseTo(27.72, 2)
    expect(sdCrystal([0, 0, 0], tune)).toBeCloseTo(0, 2)
    // The glass over it is a thin wedge — 11px from pavilion to crown —
    // which is why `chamferPx` has to keep it well inside the girdle.
    const d2 = sdInner2(0, 0, tune)
    expect(topAt(d2, tune) - bottomAt(d2, tune)).toBeCloseTo(11.2, 1)
  })

  it('reads the page 63px away from where the hand is', () => {
    const f = frameAt(W / 2, H / 2)
    const [bx, by] = bendAt(W / 2, H / 2, f, tune, EYE)
    expect(Math.hypot(bx, by)).toBeCloseTo(63.0, 1)
  ...
  it('moves 0.14px under a 2px hand tremor, and this is the chamfer', () => {
    // The number that decides whether any of this is usable. A pointer whose
    // correction moves further than the hand did is not a pointer.
    //
    // Measured 2026-08-25 on the FLAT version of this scene, whose hotspot
    // sat at a polygon vertex: 8.37px of bend and 10.65px of tremor, a ratio
    // of 1.27 — the target moved further than the correction. Rounding the
    // outline did not help, and the ratio held at 1.27 at every radius from
    // 2 to 60, because a polygon vertex is a singular point of its own
    // distance field and its gradient turns through the whole exterior angle
    // however far away the outline is placed. Grinding a flat face across
    // the point puts the hotspot under a HALF-PLANE, whose gradient is
    // constant, and tracing a real solid rather than displacing by a height
    // took what was left down by another factor of 6.7. The facets took it
    // down again: the hotspot looks through a PLANE on the way in and
    // another on the way out, and a plane has no curvature to ride along.
    const f = frameAt(W / 2, H / 2)
    const [bx, by] = bendAt(W / 2, H / 2, f, tune, EYE)
    const shake = tremorAt(W / 2, H / 2)
    expect(shake).toBeCloseTo(0.14, 1)
    expect(shake / Math.hypot(bx, by)).toBeLessThan(0.005)
  })
  ...
  it('holds up away from the middle, where the ray is no longer straight down', () => {
    ...
    //
    // Measured 2026-08-25 at all four screen edges: the bend runs 59 to 96px
    // and the worst tremor is 0.80px, at the right of the screen where the
    // slant is steepest. It is LARGER off-centre because the ray arrives at
    // an angle that adds to the facets' own slopes instead of meeting them
    // square.
    const f = frameAt(220, H / 2)
    const [bx, by] = bendAt(220, H / 2, f, tune, EYE)
    expect(Math.hypot(bx, by)).toBeCloseTo(65.6, 1)
    expect(tremorAt(220, H / 2)).toBeLessThan(0.3)

    const g = bendAt(1060, H / 2, frameAt(1060, H / 2), tune, EYE)
    expect(Math.hypot(g[0], g[1])).toBeCloseTo(96.2, 1)
  ...
  it('leaves through the FIRST face it meets, and reports that one', () => {
    ...
    // Longer than the 11.2px the stone stands at the hotspot: the ray enters
    // on a crown facet, is bent off the vertical there, and takes a slanted
    // path down to the pavilion.
    expect(hit!.through).toBeCloseTo(12.1, 1)
    // What is left after Fresnel takes its share at both faces and the glass
    // absorbs over 12px. The 15% that does not come this way is not lost —
    ...
    expect(hit!.weight).toBeCloseTo(0.85, 2)
  })
})
...
describe('the bend', () => {
  ...
  it('is nowhere near the cap except at the rim, where it is the cap that answers', () => {
    // The cap is a guard, not a scale on the effect. Measured 2026-08-25
    // over the 40,264 pixels the crystal covers at rest, with the clamp
    // lifted: the median displacement is 63px, the 99th percentile is 67,
    // and only SEVEN pixels — 0.017% — want more than `maxBendPx`.
    //
    // Those seven are the rim. A ray arriving there meets the crown at a
    // grazing angle, bounces off the pavilion instead of crossing it, and
    // leaves pointing almost along the page. Its exit is hundreds of pixels
    // away and moves by hundreds more when the hand moves by one. Uncapped,
    // those pixels smear the whole page into the silhouette's edge.
    //
    // This test steps 2px, so it sees two of the seven.
    const f = frameAt(W / 2, H / 2)
    let worst = 0
    let hits = 0
    let capped = 0
    ...
    all.sort((p, q) => p - q)
    expect(hits).toBeCloseTo(10030, -2)
    expect(all[Math.floor(all.length * 0.5)]).toBeCloseTo(62.9, 0)
    expect(all[Math.floor(all.length * 0.99)]).toBeCloseTo(67.0, 0)
...
describe('the hand', () => {
  ...
  it('settles a 400px jump in 31 frames, because the spring is overdamped', () => {
    ...
    // Critical damping for followK 900 is followD 60. The committed 82 is
    // past it, so the body never overshoots and it takes half a second to
    // arrive: 31 frames at 60Hz against the 12 a critically damped spring
    // would need. That is a slower, heavier cursor on purpose. What the
    // test still holds is that it ARRIVES — an underdamped spring would
    // cross the mark and come back, and this one is pinned not to.
...
describe('the light', () => {
  it('comes over the left shoulder and travels down into the page', () => {
    ...
    // tan(elevation)` = 26px against a solid 177px across, so the whole
    ...
    expect(offset).toBeCloseTo(25.7, 1)
...
describe('the GLSL is the same function', () => {
  ...
  it('weighs each exit by Schlick on the OUTGOING angle, both copies', () => {
    ...
    // Head on, glass hands back its 5.6%; edge on, all of it.
    expect(schlick(1, tune.ior)).toBeCloseTo(0.056, 3)
