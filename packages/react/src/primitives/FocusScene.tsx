...
export function FocusScene({
  ...
  const bundle = useMemo(() => {
    ...
    const step = (ring: string[], from: string | null, dir: 1 | -1): string | null => {
      ...
      // noUncheckedIndexedAccess: the `ring.length === 0` guard above proves
      // every index below is in bounds — 0 and length-1 trivially, and the
      // modulo result of (idx + dir + ring.length) always lands in
      // [0, ring.length) once ring.length >= 1. Same reasoning at the other
      // `ring[0]!` added below (the scene-level Enter/F2 handler).
    ...
    const onClick = (e: MouseEvent) => {
      // Voice stance: this listener wants the RELAYED voice only, and
      // gets it structurally — a trusted click's target is the
