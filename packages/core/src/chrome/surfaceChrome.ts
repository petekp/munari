...
// layout box, and the rasterizer only captures the box). Worse, the corners
...
//     the drawn subtree (walking a single-child chain, because a mounted
//     component's own root sits some containers deep).
//   - A consumer re-measures on the compositor's own paint signal, so a style
//     change reaches the mask on the frame its pixels change, and an idle
//     surface never measures at all (`chromeEquals` is what lets it drop a
//     re-measure that came back identical — "style-equality dropping").
//   - The radius becomes an analytic signed distance (`surfaceRadiusSd`):
 ...
 * single-element-child chain (a mounted component's own root can sit a few
 * containers deep) and take radii from the first element that has any,
 * shadow from the first that casts one.
 *
...
/** Value equality on radii + shadow layers — the "did anything change" gate a paintCount-keyed re-measure needs to drop a no-op update. */
