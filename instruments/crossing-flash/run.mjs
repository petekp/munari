...
// while custody crosses between the page and the canvas?
//
// The library now guarantees this in law (packages/core crossing: at
// every phase, someone presents) and in React (useCustodyCrossing
// publishes both swaps as single commits). But the law is a reducer and
...
//   did the ink stand still while custody changed hands?
...
// every frame holds custody — the eye caught it before any instrument
// did (2026-08-13). Then, with the float at its ceiling, carry rounds
...
//
// The third question is presentation custody (crossingPresentation).
...
// faults are the two tails of one number. Letters in NOBODY's custody
...
// platform imposes: for ~250ms after a custody return it emits nothing
...
// of percent at full amplitude; a flash removes ALL of it and a white
...
try {
  ...
  // The return window's own channel. After a custody return the
  ...
  // the same breathing word under page custody, which is what lets the
  // carry clause co-register them (see the calibration there).
  await sleep(350)
  ...
  // Full round trips: page → renderer → page. The settle dwell, the
  // ramp up, a beat of full-depth bob, the ramp down, the landing swap.
  for (let i = 0; i < ROUNDS; i++) {
    t0 = Date.now()
    await toggle()
    await waitPhase('renderer')
    await sleep(700)
  ...
  // Reversal one: abandon a lift mid-dwell. The DOM never released, so
  ...
  // released; the amplitude must climb back from where it was.
  t0 = Date.now()
  await toggle()
  await waitPhase('renderer')
  await sleep(250)
  ...
  // inside amplitude ≤ ~0.26, where bob and wobble pivot about centers
  ...
  const swaps = flips.filter(
    (f) => f.from === 'lifting' && f.to === 'renderer' && f.t < tCarry,
  )
  if (!swaps.length)
  ...
  // handoff. A parabola is fitted to the centroid over the 300ms before
  // the flip (the six-letter breath is a sum of slow cosines; over a
  // window this short a quadratic tracks it to a fraction of a px, and
  // extrapolates a couple hundred ms onward at well under half a px of
  // curvature error — both window lengths are set by that reach) and
  // evaluated at the frames just after; the median deviation is the
  // discontinuity. The after-frames come from two channels by
  // direction. A forward swap reads the passive cast. A backward swap
  // CANNOT — the cast is dark for ~250ms after a custody return while
  // the screen runs on (platform.md item 13) — so it reads the burst of
  // forced composites taken inside that window. A carry that breaks —
  // twins parked flat, or a clock apart — jumps the centroid by the
  // instantaneous float, several px at the ceiling. 1.5 CSS px: fit
  // residual + codec + the landing ramp's last breath, far below the
  // broken-carry signal.
  const CARRY = 1.5
  const quad = (pts) => {
  ...
  // same breathing word under page custody — and corrects every
  ...
  const carrySwaps = flips.filter(
    (f) =>
      f.t >= tCarry &&
      ((f.from === 'lifting' && f.to === 'renderer') ||
        (f.from === 'landing' && f.to === 'dom')),
  )
  ...
  for (const [i, f] of carrySwaps.entries()) {
    const forward = f.to === 'renderer'
    const dir = forward ? 'dom→gl' : 'gl→dom'
    const before = series
      .filter((s) => s.cx !== null && s.t * 1000 >= f.t - 300 && s.t * 1000 <= f.t - 30)
      .map((s) => ({ t: s.t * 1000, cx: s.cx, cy: s.cy }))
    const after = forward
      ? series
          .filter((s) => s.cx !== null && s.t * 1000 >= f.t + 30 && s.t * 1000 <= f.t + 180)
          .map((s) => ({ t: s.t * 1000, cx: s.cx, cy: s.cy }))
      : shotSeries
          .filter((s) => s.cx !== null && s.t >= f.t + 20 && s.t <= f.t + 260)
          .map((s) => ({ t: s.t, cx: s.cx - cal.dx, cy: s.cy - cal.dy }))
    ...
    const err = median(after.map((s) => Math.hypot(px(s.t) - s.cx, py(s.t) - s.cy)))
    console.log(
      `  carry swap ${i + 1} (${dir})  path continued within ${err.toFixed(2)} css px ` +
  ...
  console.log(
    `\ncrossing-flash: ${problems.length === 0 ? 'PASS — the word held custody, registration, and its breath through every frame' : 'FAIL'}`,
  )
