...
// deliberate and this sketch should not tax every other scene's boot.
...
export function ensureLogoFonts() {
...
export function glyphPaint(pose: LetterPose): React.CSSProperties {
...
/** 1 world unit = 1 CSS px, same rig as the veil — with the projection
 *  axis through the WORD, not the viewport center.
 *
 *  The camera sits over the word's center and setViewOffset slides the
 *  image back, so every world point still lands on its own CSS pixel at
 *  z = 0. What moves is the vanishing point: a letter's depth bob now
 *  displaces it laterally in proportion to its distance from the word's
 *  center, not from mid-screen. The fault, 2026-09-01: the nav wordmark
 *  sits ~640 px off the viewport axis of its full-viewport canvas, and
 *  there the ±26 px z bob leveraged into ~±15 px of sideways wander per
 *  letter — the word looked like it was flying apart while the same
 *  knobs read as a calm breath on the centered bench. */
function PixelCam({ center }: { center: { x: number; y: number } }) {
  ...
  useEffect(() => {
    // The word center in world units (origin mid-screen, y up).
    const wx = center.x - size.width / 2
    const wy = size.height / 2 - center.y
    camera.fov = FOV
    camera.position.set(wx, wy, cameraDistance(size.height, FOV))
    camera.near = 1
    camera.far = camera.position.z * 3
    camera.setViewOffset(size.width, size.height, -wx, wy, size.width, size.height)
    camera.updateProjectionMatrix()
  }, [camera, size.width, size.height, center.x, center.y])
  return null
}
...
export function MatterWord({
  ...
  // Where the projection axis goes (PixelCam): the mean of the letter
  // centers, in CSS viewport px.
  const wordCenter = useMemo(() => {
    const n = metrics.boxes.length
    let x = 0
    let y = 0
