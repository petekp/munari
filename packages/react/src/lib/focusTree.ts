  ...
  return {
    ...
    recall(valid) {
      while (stack.length > 0) {
        // noUncheckedIndexedAccess: the `while` guard above proves
        // `stack.length > 0`, so index `length - 1` is in bounds — same
        // loop/length-guard reasoning backs every other `!` added in this
        // file (interiorBoundary, readingOrder below).
