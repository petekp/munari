...
describe('logo choreography (logoLaw)', () => {
  ...
  it('never re-rolls a letter into the face, color, or material it already wears', () => {
    const r = makeRng(1)
    let pose = rollPose(r, { fonts: [], colors: [], materialIndices: [] }, K)
    for (let i = 0; i < 300; i++) {
      const next = rollPose(
        r,
        { fonts: [pose.font], colors: [pose.color], materialIndices: [pose.materialIndex] },
        K,
      )
      expect(next.font).not.toBe(pose.font)
      expect(next.color).not.toBe(pose.color)
      expect(next.materialIndex).not.toBe(pose.materialIndex)
      pose = next
    }
  })
  ...
  it('dodges the neighbors it was told about', () => {
    const r = makeRng(2)
    for (let i = 0; i < 300; i++) {
      const avoid = { fonts: [0, 3, 5], colors: [1, 2, 6], materialIndices: [0, 2] }
      const pose = rollPose(r, avoid, K)
      expect(avoid.fonts).not.toContain(pose.font)
      expect(avoid.colors).not.toContain(pose.color)
      expect(avoid.materialIndices).not.toContain(pose.materialIndex)
    }
  })
  ...
  it('falls back to the full deck when over-constrained instead of throwing', () => {
    ...
    const all = {
      ...
      materialIndices: LOGO_MATERIALS.map((_, i) => i),
    }
    ...
    expect(pose.materialIndex).toBeGreaterThanOrEqual(0)
    expect(pose.materialIndex).toBeLessThan(LOGO_MATERIALS.length)
  })

  it('only deals weights the chosen face actually ships', () => {
    const r = makeRng(4)
    for (let i = 0; i < 300; i++) {
      const pose = rollPose(r, { fonts: [], colors: [], materialIndices: [] }, K)
  ...
  it('keeps every pose inside the knobs that shaped it', () => {
    const r = makeRng(5)
    for (let i = 0; i < 300; i++) {
      const p = rollPose(r, { fonts: [], colors: [], materialIndices: [] }, K)
  ...
  it('seeds a word where no two neighbors share a face, color, or material', () => {
    for (let seed = 10; seed < 40; seed++) {
      ...
      for (let i = 1; i < word.length; i++) {
        ...
        expect(word[i].materialIndex).not.toBe(word[i - 1].materialIndex)
      }
    }
  })
})
