...
try{
 ...
 const appearance=()=>page.evaluate(()=>({width:innerWidth,height:innerHeight,light:document.querySelector('.home-light').style.transform}))
 const reference=await appearance()
 assert.ok(clip.x>=0&&clip.y>=0&&clip.x+clip.width<=reference.width&&clip.y+clip.height<=reference.height,'The full comparison crop must fit in the viewport')
 // Beyond-viewport capture briefly resized Chrome to 1×1 and clamped the
 // movable light into the corner. Keep the crop inside the unchanged viewport.
 ...
 const hidden=await capture(),negative=await textureClarity(page,before,hidden)
